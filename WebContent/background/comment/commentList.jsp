...
<div class="data_list backMain">
	...
	<div class="search_content" style="vertical-align: middle;padding-right: 20px;">
		<div style="float: left;padding-top: 10px;">
			<button class="btn btn-mini btn-danger" type="button" onclick="commentsDelete()">대량삭제</button>
		</div>
		<div style="float: right;">
			<form action="${pageContext.request.contextPath}/comment?action=backList" method="post">
				댓글날자：<input type="text" id="s_bCommentDate" name="s_bCommentDate" class="Wdate" onclick="WdatePicker()" style="width: 100px;" value="${s_bCommentDate }"/>
				&nbsp;부터&nbsp;<input type="text" id="s_aCommentDate" name="s_aCommentDate" class="Wdate" onclick="WdatePicker()" style="width: 100px;" value="${s_aCommentDate }"/>
		...
		<table class="table table-hover table-bordered">
			...
			<c:forEach var="commentBack" items="${commentBackList }" varStatus="status">
				<tr>
					...
					<td><a title="${commentBack.articalTitle }" target="_blank" href="artical?action=show&articalId=${commentBack.articalId }">${fn:substring(commentBack.articalTitle,0,15) }...</a></td>
					<td><fmt:formatDate value="${commentBack.commentDate }" type="date" pattern="yyyy-MM-dd"/></td>
					<td><button class="btn btn-mini btn-danger" type="button" onclick="commentDelete(${commentBack.commentId})">삭제</button></td>
				</tr>
			</c:forEach>
		</table>
	</div>
